...
total_img_lists = train_img_lists + val_img_lists
total_labels = y_train + y_val


X_train, X_test, y_train, y_test = train_test_split(total_img_lists, total_labels, test_size=0.03, random_state=42)
...
train_dataset = FGV5Data(list(X_train), train_transforms)
train_dataset_no_aug = FGV5Data(list(X_train), train_transforms_no_color_aug)
train_dataset_warm = FGV5Data(list(X_train), train_transforms_warm)
...
val_dataset = FGV5Data(list(X_test), val_transforms)

val_data = DataLoader(val_dataset, batch_size=20, shuffle=False, drop_last=False, num_workers=args.num_workers)
...
def train():
	...
	patience = 0
	exp_lr_scheduler = StepLR(optimizer, step_size=2, gamma=0.7)
